...
- **Health/fitness liability** — mitigated by Apache-2.0's no-warranty terms + `DISCLAIMER.md`. *Recommended:* also surface a short disclaimer/consent line **in the UI** (first run or footer), not just in docs. *(I can implement this — just ask.)*
...
- [ ] In-app disclaimer / consent banner (ties the legal docs to the actual UI).
- [ ] Vendor fonts + MediaPipe model for fully offline, no-third-party-call operation.
